...
Article클래스와 ArticleEventHanlder클래스는 연관, 집합 관계를 가진다.
...
* Content와 Article에 이벤트 소싱 로직을 추가하기 위해 이미 구현한 코드를 고쳐야 하나(새로운 코드를 끼워 넣어야 하나)?
  ```java
  public void modify(ArticleContent articleContent) {
    Assert.notNull(articleContent, "articleContent must not be null.");
...
* 사실 이 분이 좀 마음에 걸린다. 어쨋든 Content타입에 변화가 생겼고, Content의 도메인 로직과 무관한 메소드들이 공개 되어 호출되면 오류가 발생할 여지가 생겼다.
